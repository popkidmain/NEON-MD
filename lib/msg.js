...
const sms = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        // Fixed Quoted and ContextInfo access
        ...
        if (m.quoted) {
            ...
            if (!quoted.viewOnceMessageV2) {
                ...
                m.quoted.sender = m.msg?.contextInfo?.participant ? m.msg.contextInfo.participant.split(':')[0] + '@s.whatsapp.net' : ''
    ...
    m.reply = async (content, opt = { packname: "Secktor", author: "SamPandey001" }, type = "text") => {
        switch (type.toLowerCase()) {
            ...
            case "image":
                return await conn.sendMessage(m.chat, { [Buffer.isBuffer(content) ? 'image' : 'image']: (Buffer.isBuffer(content) ? content : { url: content }), ...opt }, { ...opt });
            case "video":
                return await conn.sendMessage(m.chat, { [Buffer.isBuffer(content) ? 'video' : 'video']: (Buffer.isBuffer(content) ? content : { url: content }), ...opt }, { ...opt });
            case "audio":
                return await conn.sendMessage(m.chat, { [Buffer.isBuffer(content) ? 'audio' : 'audio']: (Buffer.isBuffer(content) ? content : { url: content }), ...opt }, { ...opt });
            case "sticker":
                ...
                if (mime == "image/webp") {
                    ...
                    if (mime.split("/")[0] === "video" || mime.split("/")[0] === "image") {
    ...
    m.senddoc = (doc, type, id = m.chat, option = { mentions: [m.sender], filename: "Document", mimetype: type }) => conn.sendMessage(id, { document: doc, mimetype: option.mimetype, fileName: option.filename, contextInfo: { mentionedJid: option.mentions } }, { quoted: m })
